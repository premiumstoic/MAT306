  ...
  return (
    <main className="min-h-screen p-4 md:p-8 xl:p-12 max-w-[1600px] mx-auto space-y-16">
      ...
      <header className="space-y-6 text-center md:text-left">
        <div className="inline-flex items-center rounded-full px-4 py-1.5 text-sm font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 ring-1 ring-inset ring-emerald-500/20">
          MAT 306 — Homework 1
        </div>
      ...
      <section id="section-intro" className="space-y-6">
        <h2 className="text-3xl font-bold border-b border-black/10 dark:border-white/10 pb-4">1. Aim & Introduction</h2>
        <div className="grid md:grid-cols-2 gap-8 items-start">
          <div className="space-y-4 text-muted-foreground leading-relaxed">
            ...
            <div className="text-center bg-black/5 dark:bg-white/5 py-4 rounded-xl border border-black/10 dark:border-white/10 my-6">
          ...
          <div className="glass-panel p-4 rounded-3xl border border-black/10 dark:border-white/10 shadow-2xl">
      ...
      {/* Section 2 */}
      <section id="section-methods" className="glass-panel p-8 rounded-3xl space-y-6 bg-gradient-to-br from-black/5 dark:from-white/5 to-transparent border border-black/10 dark:border-white/10">
      ...
      <section id="section-results" className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-black/10 dark:border-white/10 pb-4 gap-4">
          <div>
          ...
          <div className="flex bg-black/5 dark:bg-white/5 p-1 rounded-xl border border-black/10 dark:border-white/10 shrink-0">
            <button
              onClick={() => setSimLength("20ns")}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${
                simLength === "20ns"
                  ? "bg-black/10 dark:bg-white/20 text-foreground shadow-lg"
        ...
        <div className="glass-panel p-8 rounded-3xl border border-black/10 dark:border-white/10 max-w-4xl mx-auto mt-12 shadow-2xl shadow-emerald-500/10">
          <h3 className="text-2xl font-bold mb-6 text-emerald-600 dark:text-emerald-400">Energy Summary</h3>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-black/10 dark:border-white/10">
                  <th className="py-3 font-semibold text-foreground">Conformational State</th>
                  <th className="py-3 font-semibold text-foreground">Topological Feature</th>
                  <th className="py-3 font-semibold text-emerald-600 dark:text-emerald-400">Relative Free Energy (ΔG)</th>
                </tr>
              </thead>
              <tbody className="text-muted-foreground">
                {ENERGY_ROWS.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={row.key}
                      className={`transition-colors duration-300 ${i < ENERGY_ROWS.length - 1 ? "border-b" : ""} ${isActive ? "" : "border-black/5 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/5"}`}
                      style={isActive ? { borderColor: color + "30", background: color + "12" } : {}}
                    >
                      <td className="py-4 font-bold transition-colors duration-300" style={{ color: isActive ? color : undefined }}>
                        {!isActive && <span className="text-foreground">{row.label}</span>}
                        {isActive && (
                          <>
                            <span className="inline-block w-1.5 h-1.5 rounded-full mr-2 mb-0.5" style={{ background: color }} />
                            {row.label}
                          </>
                        )}
                      </td>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-black/10 dark:border-white/10 pb-4 gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              ...
              <span className={`ml-2 text-xs font-semibold px-2 py-0.5 rounded-full ${simLength === "20ns" ? "bg-orange-500/20 text-orange-400" : "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400"}`}>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-300 ${
                  activeConformation === key
                    ? "bg-black/10 dark:bg-white/20 text-foreground scale-105 shadow-lg"
                    : "bg-black/5 dark:bg-white/5 text-muted-foreground hover:bg-black/10 dark:hover:bg-white/10 hover:scale-105"
                }`}
              >
      ...
      <section id="section-conclusions" className="space-y-6">
        <h2 className="text-3xl font-bold border-b border-black/10 dark:border-white/10 pb-4">4. Conclusions</h2>
        <div className="glass-panel p-6 rounded-2xl space-y-4 text-muted-foreground leading-relaxed border border-black/10 dark:border-white/10">
          <p>
